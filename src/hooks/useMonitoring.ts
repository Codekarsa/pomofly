...
export function useMonitoring() {
  ...
  const recordMetric = useCallback(
    (metric: string, value: number, tags: string[] = []) => {
  ...
  const timeFunction = useCallback(
    <T>(name: string, fn: () => T, tags: string[] = []): T => {
...
>>>>>>> dc46537 (feat: implement comprehensive Prettier code formatting integration)
  ...
  const trackApiCall = useCallback(
    (endpoint: string, method: string, status: number, duration: number) => {
      ...
      recordMetric(`api_call_${status >= 400 ? 'error' : 'success'}`, 1, [
        'api',
        endpoint,
        method,
      ]);
...
<<<<<<< HEAD
  const monitorApiCall = useCallback(async <T>(
    ...
    try {
      ...
      const duration = performance.now() - start;
      const status = (error as { status?: number; response?: { status?: number } })?.status || (error as { status?: number; response?: { status?: number } })?.response?.status || 500;
  ...
  const monitorApiCall = useCallback(
    async <T>(
      ...
      try {
        ...
        const duration = performance.now() - start;
        const status =
          (error as any)?.status || (error as any)?.response?.status || 500;
        trackApiCall(endpoint, method, status, duration);
        throw error;
      }
    },
...
export function usePerformanceMonitoring() {
  ...
  const measureRender = useCallback(
    (componentName: string, renderFn: () => JSX.Element) => {
      return timeFunction(`${componentName}_render`, renderFn, [
        'component_render',
      ]);
    },
    [timeFunction]
  ...
  const recordInteractionTime = useCallback(
    (interaction: string, startTime: number) => {
      const duration = performance.now() - startTime;
      recordMetric(`${interaction}_time`, duration, [
        'interaction',
        interaction,
      ]);
    },
